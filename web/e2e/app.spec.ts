...
test.describe("EvacLogix presentation site", () => {
  test("loads home with landmarks, mission, and CTA flow", async ({ page }) => {
    ...
    await expect(
      page.getByText(
        "Explore evacuation behavior through an interactive building-safety simulation."
      )
    ).toBeVisible()
  ...
  test("routes to How It Works and shows high-level explanatory panels", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: "A high-level look at what the simulation is meant to show."
      })
    ).toBeVisible()
  ...
  test("keeps sandbox editor hidden from navigation but addressable by profile route", async ({
    page
  }) => {
    await page.goto("/")
